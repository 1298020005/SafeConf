#!/usr/bin/env python3
"""Package E14 official sciplex3 full-743 drug-dose gene1000 probe."""
...
RUNTIME = ROOT / "runtime" / "e14_sciplex3_official_3cell_panel_full743_gene1000_20260707"
OUT = ROOT / "docs" / "实验结果" / "E14_sciplex3_full743_gene1000_20260707"
TABLES = OUT / "tables"
FIGURES = OUT / "figures"
REPORTS = OUT / "reports"
...
def write_reports(s: dict[str, Any]) -> None:
    ...
    report = f"""# E14 官方 sciplex3 full-743 gene1000 panel
...
- `prediction_magnitude_risk` aligned Spearman = {float(magnitude['direction_aligned_spearman']):.3f}，方向仍弱。
...
"""
    (REPORTS / "E14_SCIPLEX3_FULL743_GENE1000_REPORT.md").write_text(report, encoding="utf-8")
...
<html lang="zh-CN">
<head><meta charset="utf-8"><title>E14 sciplex3 full-743 gene1000</title>
<style>
...
<section class="hero"><h1>E14 sciplex3 full-743 gene1000</h1><p class="lead">三细胞系全部 743 个共享 drug-dose 已跑通；本轮使用 1,000 genes，是 full perturbation / low-gene 快速验证。</p><div class="metrics">{cards_html}</div></section>
...
<section class="card warn"><h2>4. 边界</h2><p>本轮是 1,000-gene 快速验证。5,000-gene full-743 仍需进一步优化 task-fold 级特征缓存。</p></section>
<section><h2>5. 图件</h2><div class="figgrid">{fig_html}</div></section>
</main></body></html>
"""
    (REPORTS / "E14_SCIPLEX3_FULL743_GENE1000.html").write_text(html_doc, encoding="utf-8")
...
def main() -> None:
    ...
    status = {
        ...
        "provenance_note": (
            "Runtime was generated from the listed HEAD plus the E14 feature-vectorization "
            "working-tree changes committed together with this package."
        ),
    ...
    (OUT / "README_先看这个.md").write_text(
        "# E14 sciplex3 full-743 gene1000\n\n"
        "先看：\n\n"
        "- `reports/E14_SCIPLEX3_FULL743_GENE1000.html`\n"
        "- `reports/E14_SCIPLEX3_FULL743_GENE1000_REPORT.md`\n\n"
        "这个目录只保存可汇报的轻量结果。原始矩阵在 `runtime/e14_sciplex3_official_3cell_panel_full743_gene1000_20260707/`。\n",
        encoding="utf-8",
    )
    print(f"Wrote E14 package to {OUT.relative_to(ROOT)}")
