#!/usr/bin/env python3
"""Profile TxPert-Exphormer under the frozen E201 whole-context contract.

This E205 adapter preserves the E201 blind data view and training rules while
replacing only the perturbation propagation architecture. It never constructs
the held-out target treatment dataset during training.
"""
...
def main() -> None:
    ...
    try:
        ...
        with initialize_config_dir(
            ...
            architecture_cfg = hydra_compose(config_name="config-exphormer")
        # Preserve the whole-context data contract, no-basal design, STRING graph,
        # optimizer, decoder and evaluation rules from config-x-cell-gat. Only
        # the perturbation propagation block comes from config-exphormer.
        ...
        if (
            cfg.model.no_basal_model is not True
            or cfg.model.pert_model.model_type != "exphormer"
            or cfg.model.pert_model.layer_type != "exphormer_w_mpnn"
            or tuple(cfg.datamodule.train_cell_types) != tuple(source_contexts)
            or cfg.datamodule.test_cell_type != args.target
        ):
            raise TrainingFailure("E205 architecture/data contract composition failed")
        metadata.update(
            {
                ...
                "resolved_graph_config": OmegaConf.to_container(
                    cfg.graph, resolve=True
                ),
                "heldout_contract": "whole-cell target; zero target perturbations in training",
            }
        )
        write_json(run_dir / "E205_RUN_STATUS.json", metadata)
        set_seed(args.seed)
        if not torch.cuda.is_available():
            raise TrainingFailure("CUDA is required for E205 training")
