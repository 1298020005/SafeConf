#!/usr/bin/env python3
"""Run the frozen public TxPert K562 cross-cell checkpoint or baseline.

The upstream entry point downloads every Zenodo file automatically.  This
adapter calls the pinned source directly, refuses overwrite, and keeps all raw
predictions outside the SafeConf repository.
"""
...
def require_repo(repo: Path) -> None:
    required = (
        repo / "main.py",
        repo / "configs/config-x-cell-gat.yaml",
        repo / "configs/config-x-cell-baseline.yaml",
...
def run_leakage_smoke(args: argparse.Namespace) -> dict:
    ...
    cfg = compose(
        args.txpert_repo,
        "config-x-cell-gat",
        ["mode=predict", f"datamodule.batch_size={min(args.batch_size, 16)}"],
    )
    ...
    def array_hash(tensor: torch.Tensor) -> str:
        raw = np.ascontiguousarray(tensor.detach().cpu().numpy()).tobytes()
        return hashlib.sha256(raw).hexdigest()

    control_before = array_hash(batch.control)
    truth_before = array_hash(batch.x)
    with torch.no_grad():
    ...
    exact = bool(torch.equal(prediction_before, prediction_after))
    delta = float(torch.max(torch.abs(prediction_before - prediction_after)).cpu())
    if not exact or delta != 0.0:
        raise RuntimeError(
            f"target leakage smoke failed: exact={exact}, max_abs_delta={delta}"
        )
    if control_before != array_hash(batch.control):
        raise RuntimeError("control tensor changed during leakage smoke")
    ...
    return {
        ...
        "truth_hash_before_zeroing": truth_before,
...
def run_general_baseline(args: argparse.Namespace) -> dict:
    ...
    controls = (
        torch.stack([test_ds._get_control_data(i) for i in range(len(test_ds))])
        .detach()
        .cpu()
        .numpy()
    )
    ...
    if not (predictions.shape == truths.shape == controls.shape):
        raise RuntimeError("cross-cell baseline matrices are not aligned")
    labels = [
        f"{ct}_{pert}_{dose}"
    ...
    return {
        ...
        "config_name": "config-x-cell-baseline",
        "seed": int(cfg.seed),
        "batch_size": int(cfg.datamodule.batch_size),
        "task_type": str(cfg.datamodule.task_type),
...
def main() -> None:
    ...
    (args.save_dir / "E200_ADAPTER_RUN.json").write_text(
