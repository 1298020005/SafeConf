# Codex 当前接手说明

不要把 `agents/grok/CODEX_READ_THIS.md` 或其他 Agent 原始输出当作当前事实。它们是当时的建议和运行记录，保留用于追溯。

请依次读取：

1. `../docs/学习导航/00_当前项目手把手学习_20260905.md`
2. `../docs/实验结果/CURRENT_RESEARCH_DECISION_20260905.md`
3. `../docs/实验结果/E201_txpert_multitarget_retraining_20260802/formal_core_evaluation/reports/E201_CORE_REPORT.md`
4. `../docs/实验结果/E204_risk_guided_training_20260830/PROFILE_ACCEPTANCE_20260905.md`

当前状态（2026-09-22）：E201 已完成正式评价；E202 主门失败；E204 训练加权仍是独立问题；E205 已完成但其原始 pooled 增量需要结合 E220 同口径审计解读；E208 Jiang24 正式训练仍在进行，测试真值未授权；E221/E222 为已完成的历史数据审计，均未改 E208 协议；E223/E224 在 E187 上完成了幅度锚定与场景门控的嵌套开发审计。

修改或续跑前先查看根目录 `START_HERE_FOR_AGENTS.md` 的审计纪律。
