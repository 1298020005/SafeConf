...
SafeConf 的来源背景证据只使用本合同保留的 `train` 细胞：每个来源状态的
扰动质心减去同状态 control 质心，状态等权；来源细胞量使用资源训练层实际保留
的扰动细胞数。`val`、测试 control 和测试扰动表达均不进入来源证据。这样上游模型
和风险层使用同一资源训练合同，不借用模型未训练过的额外表达数据。
