...
E225–E227三轮先登记再执行，使用同一份历史开发资料：8196条重复评估记录、1083个去重研究–背景–扰动条件、4个研究。E228/E230则使用E201的四目标背景资料，两类来源不能混算。研究/目标簇重采样区间均作描述性解释；四簇不足以构成广泛外推保证。主结果不因表现差而删除。
...
| E228，12次拟合 | 仅评分器误差标签轮留；训练特征来源仍接触外层目标，撤回全流程隔离解释 | +0.017608 | [−0.015783, +0.054148] | 未通过；来源审计另有限制 |
| E230，无误差模型拟合 | 实际抽样来源细胞，固定80%幅度＋20%五项风险 | +0.011995 | [+0.003454, +0.020535] | 4/4正向，未达预定+0.02 |
...
5. **E228需要撤回全流程未见背景的解释。** 其他目标的历史特征引用了当前外层目标的真实表达；只删除训练表中的外层误差标签不够。数字保留作评分器层诊断，不能用于声称新背景无泄漏泛化。原E201逐目标排除自身来源的隔离仍成立，不混淆两者。[审计表](../../实验结果/E229_quality_gated_history_router_20260922/PROVENANCE_AUDIT.json)。
...
**E230已运行完成。** 方案先提交到双远程（116cc9f），然后读取来源H5AD进行真实细胞抽样。E187的25%/50%/75%/100%同时改变上游训练条件，不能冒充纯历史库容量实验；本轮没有用手工缩放特征替代抽样。

4目标、1808主任务、三个抽样种子；分别改变来源背景数1/2/3与每背景细胞比例25%/50%/100%。另固定总30细胞比较背景多样性，四目标各243个主任务满足这个预定覆盖条件。最大容量的原始特征与E201冻结值逐项复算一致（最大误差约数十亿分之一）；没有读取目标表达构造特征。

| 预定主要比较 | 20%复核效用差值 | 描述性95%区间 | 正向目标 |
|---|---:|---|---:|
| 三来源100%细胞 vs 三来源25%细胞 | +0.005070 | [+0.000254,+0.010894] | 4/4 |
| 固定总30细胞，三背景 vs 一背景 | +0.016115 | [−0.015182,+0.052823] | 3/4 |
| 完整历史组合评分 vs 幅度 | +0.011995 | [+0.003454,+0.020535] | 4/4 |

主评分相对固定80%幅度＋20%分歧的辅助差值为+0.029657、4/4正向；不能将这个辅助比较改成主对照。所有任务、配置、10%/20%/30%预算都保留。四背景并非四个全新外部研究，E201标签先前已公开，因此本轮属于机制开发，不是新盲测。

![真实历史库容量结果](./figures/05_real_history_capacity.png)

[E230报告与预定门](../../实验结果/E230_history_capacity_20260922/REPORT.md) · [逐目标主比较](../../实验结果/E230_history_capacity_20260922/FULL_CAPACITY_COMPARATORS.csv) · [完整曲线数据](../../实验结果/E230_history_capacity_20260922/CAPACITY_CURVES.csv) · [复算审计](../../实验结果/E230_history_capacity_20260922/RESULT_AUDIT.json)。

目前可直接支持：现有公式对增加历史细胞有小幅响应，额外来源背景的作用取决于目标。仍需验证来源质量处理和相对强学习基线的增益，不能从“多采细胞略好”推成“库越大越好”。
...
本项目当前尚未闭合这条证据链；本轮给出实际结果和来源审计，不以“后面一定能发”替代结果。某个消融失败就保留记录、在下一轮按证据简化模块；重新提出的假设继续属于开发，不能无限使用同一测试集直到显著。
...
python tools/scripts/build_e230_history_figures.py
python tools/scripts/audit_e230_history_capacity.py \
  --result-dir docs/实验结果/E230_history_capacity_20260922 \
  --truth /path/to/E201_TASK_METRICS.csv
```
