...
| 条件化方法 | E225跨研究、E226本地校准、E227非线性对照、E228目标背景留出均完成；四个预定开发门均未通过 | 不能称已验证的通用自适应系统 |
...
| E228目标背景留出 | E201四个目标背景轮流留出，只在其他三个背景的已知误差上拟合原始历史字段 | 相对幅度+0.01761，但仅2/4目标为正、区间跨0；历史增量未闭合 |
...
本次已完成四轮、1852次轻量拟合（160+960+720+12），其中E228使用独立的E201目标背景留出表；这些不是1852组独立生物实验。完整可下载图、相对路径、公式说明与复现命令集中在本包；[本次工作记录](./CHANGELOG.md)列出更正原因和Git内容。
