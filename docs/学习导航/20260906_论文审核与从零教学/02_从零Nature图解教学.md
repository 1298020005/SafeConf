...
**图 1a** 是整条流水线。左边两个盒子是输入：没被扰动的细胞，以及“敲低了哪个基因”。中间黄色盒子是 TxPert，它给出扰动后的表达预测。灰色盒子说的是：同一套 TxPert 换四个随机种子，得到四份预测。绿色盒子是 SafeConf，它在对答案前计算分歧、幅度和源域证据。粉盒子是第一种用途：优先复核高风险任务。右下灰盒子是第二种用途：训练时给难任务加权重。灰盒子上写了“尚未有正式效果”，因为 E204 还停在工程验收。
...
能讲清这六句，就可以继续读完整教程 `docs/学习导航/00_当前项目手把手学习_20260905.md` 的第 4 节以后。
